...
async function openTaskDetail(taskId) {
  ...
  try {
    ...
    // Fetch template code
    const templateResponse = await fetch(`${API_BASE}/template/${taskId}`)
    const templateData = await templateResponse.json()

    // Fetch sample data
    const dataResponse = await fetch(`${API_BASE}/sample-data/${taskId}`)
...
async function submitFile(event, taskId) {
  ...
  try {
    ...
    alert("Error submitting file. Check console for details.")
  }
}
